...
output$printer <- renderText(paste("First AI: ", input$firstDTRcompareA, ", Second AI: ", input$secondDTRcompareA))
  ...
  generateBinaryInputs2A <- reactive({
    ...
    if(input$targetDiffCheckA == FALSE && input$targetOddsCheckA == FALSE){
      ...
      if(input$targetDiffCheckA == TRUE && input$targetOddsCheckA == FALSE){
        return(list(numericInput("targetDiffA", label = "Target Difference in Success Probabilities", value = 0.1, min = 0.01, max = 0.5, step = 0.01),
      ...
      if(input$targetOddsCheckA==TRUE){
        return(list(numericInput("targetORA", label = "Target Odds Ratio of Success", value = 2, min = 0, step = 0.01),
  ...
  checkDTRinputsA <- reactive({
    test <- (input$DTRsuccA1 > 0) & (input$DTRsuccA1 < 1) & (input$DTRsuccA2 > 0) & (input$DTRsuccA2 < 1)
  ...
  output$binaryPowerA <- renderUI({
    ...
    formatResp <- as.numeric(input$respA)
    validate(
      need(input$inputSampleSizeA >= 1, paste("The provided sample size is not large enough to yield a trial in which at least one person is consistent with each DTR.", 
