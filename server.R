...
shinyServer(function(input, output, session) {
  ...
  premade <- eventReactive(input$premade.startbutton,
                           list(
                             p(
                               "The three designs below are commonly seen in the field. If your design
                             ...
                             fluidRow(
                               column(
                                 ...
                                 div(a(
                                   img(
                                     id = "D1.diagram.home",
                                     src = "images/SMARTdesignA__.gif",
                                     class = "img-responsive img-zoom",
                                     alt = "Design I"
                                   ),
                                   href = "images/SMARTdesignA__.gif",
                                   target = "_blank"
                                 )),
                                 br(),
                                 actionButton("pickTabA", "Use Design I", class = "center-block"),
                                 p(
                                   "Both responders and non-responders to first-stage treatment are re-randomized.
                                          There are 8 embedded adaptive interventions: {A,C,E}, {A,C,F}, {A,D,E}, {A,D,F},
                                          {B,G,I}, {B,G,J}, {B,H,I}, {B,H,J}.",
                                   a(
                                     "Click here for a
                                            n example from the field.",
                                     `data-toggle` = "modal",
                                     `data-target` = "#exampleAmodal",
                                     style = "color:#6b6b6b"
                                   )
                                 ),
                               ...
                               column(
                                 ...
                                 div(a(
                                   img(
                                     id = "D2.diagram.home",
                                     src = "images/SMARTdesignB__.gif",
                                     class = "img-responsive img-zoom",
                                     alt = "Design II"
                                   ),
                                   href = "images/SMARTdesignB__.gif",
                                   target = "_blank"
                                 )),
                                 br(),
                                 actionButton("pickTabB", "Use Design II", class = "center-block"),
                                 p(
                                   "Only non-responders to either first-stage treatment are re-randomized.
                                          There are 4 embedded adaptive interventions: {A,C,D}, {A,C,E}, {B,F,G}, {B,F,H}.",
                                   a(
                                     "Click here for an example from the field.",
                                     `data-toggle` = "modal",
                                     `data-target` = "#exampleBmodal",
                                     style = "color:#6b6b6b"
                                   )
                                 ),
                               ...
                               column(
                                 ...
                                 div(a(
                                   img(
                                     id = "D3.diagram.home",
                                     src = "images/SMARTdesignC__.gif",
                                     class = "img-responsive img-zoom",
                                     alt = "Design III"
                                   ),
                                   href = "images/SMARTdesignC__.gif",
                                   target = "_blank"
                                 )),
                                 br(),
                                 bsButton("pickTabC", "Use Design III", class = "center-block"),
                                 p(
                                   "Only non-responders to a particular first-stage treatment are re-randomized.
                                          There are 3 embedded adaptive interventions: {A,C,D}, {A,C,E}, {B,F,G}.",
                                   a(
                                     "Click here for an example from the field.",
                                     `data-toggle` = "modal",
                                     `data-target` = "#exampleCmodal",
                                     style = "color:#6b6b6b"
                                   )
                                 ),
  ...
  observeEvent(input$premade.startbutton,
               shinyjs::hide("premade.startbutton"))
  
  output$design1img.home <-
    renderImage(list(src = file.path("www", "images", "SMARTdesignA__.gif")), deleteFile = FALSE)
  ...
  shinyBS::createAlert(
    session,
    anchorId = "dyo-functional",
    title = "Use with Caution!",
    content = paste(
      "<p>This section is only fully functional for comparisons of",
  ...
  dyo.resultOptions <-
    callModule(resultOptions, "dyo.resultOptions")
  output$dyoresultOptions <-
    renderText(unlist(dyo.resultOptions()))
  ...
  dyo.primaryAim.rerand <- reactive({
    validate(
      need(
        ...
        text.mustRerandomize
      )
    )
    c("responders", "nonresponders")[c(input$dyo.rerand.resp == "Yes",
                                       input$dyo.rerand.nresp == "Yes")]
  })
  primaryAim <-
    callModule(primaryAim, "dyo.primaryAim", rerand = dyo.primaryAim.rerand)
        ...
        lapply(1:input$dyo.stage1.ntxt,
               function(i) {
                 disabled(numericInput(
                   paste0("dyo.stage1.", LETTERS[i], ".resprob"),
  ...
  output$dyo.rerand.respUI <- renderUI({
    list(
      checkboxGroupInput(
        "dyo.rerand.resp.whichtxt",
        label = "Select first-stage treatments for which responders are re-randomized.",
        choices = lapply(1:input$dyo.stage1.ntxt, function(i)
          LETTERS[i]),
        selected = LETTERS[1:input$dyo.stage1.ntxt],
        inline = TRUE
      )
    )
  })
  ...
  output$dyo.stage1.rprobUI <- renderUI({
    list(lapply(1:input$dyo.stage1.ntxt,
         ...
         helpText(
           paste(
             "The probability of allocation to Treatment",
             LETTERS[isolate(input$dyo.stage1.ntxt)],
  ...
  output$dyo.rerand.resp.rprobUI <- renderUI({
    list(lapply(1:input$dyo.rerand.resp.ntxt,
                function(i) {
                  numericInput(
                    paste0("dyo.rerand.resp.txt", i, ".rprob"),
                    label = paste0("Probability of allocation to Treatment ", LETTERS[i]),
                    min = 0,
                    max = 1,
                    value = NA,
         ...
         helpText(
           paste(
             "The probability of allocation to Treatment",
             LETTERS[isolate(input$dyo.rerand.resp.ntxt)],
             "will automatically update so that all allocation probabilities sum to 1."
           )
         ))
  })
  output$dyo.rerand.nresp.rprobUI <- renderUI({
    list(lapply(1:(input$dyo.rerand.nresp.ntxt),
                function(i) {
                  numericInput(
                    paste0("dyo.rerand.nresp.txt", i, ".rprob"),
         ...
         helpText(
           paste(
             "The probability of allocation to Treatment",
             isolate(input$dyo.rerand.nresp.ntxt),
             "will automatically update so that all allocation probabilities sum to 1."
           )
         ))
  ...
  observe({
    updateNumericInput(
      ...
      paste0(
        'dyo.rerand.resp.txt',
        isolate(input$dyo.rerand.resp.ntxt),
        '.rprob'
      ),
  ...
  observe({
    shinyjs::toggleState(
      "dyo.rerand.continue",
      input$dyo.rerand.resp == "Yes" | input$dyo.rerand.nresp == "Yes"
    )
  })
  ...
  stage2RTxNames <- reactive({
    validate(need(isTruthy(input$dyo.rerand.resp.ntxt), 'oops'))
  ...
  stage2NRTxNames <- reactive({
    validate(need(isTruthy(input$dyo.rerand.nresp.ntxt), 'oops'))
  ...
  dtr.names <- reactive({
    ...
    stage2nr.names <-
      lapply(1:input$dyo.stage1.ntxt, function(i) {
    ...
    namepairs <-
      do.call(rbind, lapply(1:input$dyo.stage1.ntxt, function(i) {
        expand.grid("r" = stagenames$r[[i]], "nr" = stagenames$nr[[i]])
      }))
    namepairs <- namepairs[order(namepairs$r),]
    namepairs <-
      cbind("s1" = substr(namepairs$r, 1, 1), namepairs)
  ...
  output$binaryRefInput <- renderUI({
    if (isTruthy(primaryAim() == "dtrs")) {
      validate(need(isTruthy(refDTR.substr()[1]), text.refDTRPlaceholder))
      return(list(
        numericInput(
          "refDTRProb",
  ...
  output$binaryCompInput <- renderUI({
    if (isTruthy(primaryAim() == "dtrs")) {
      validate(need(isTruthy(compDTR.substr()[1]), text.compDTRPlaceholder))
      return(list(
        numericInput(
          "compDTRProb",
  ...
  output$binaryRefCellProbs <- renderUI({
    req(input$dyoRefDTR)
    validate(if (input$dyo.stage1.resprob.eq == "Yes") {
      need(isTruthy(input$dyo.stage1.allTxt.resprob),
           text.noResponse)
    } else {
      need(
        isTruthy(input[[paste0("dyo.stage1.", refDTR.substr()[2], ".resprob")]]),
        paste0(
          "Please provide the probability of response to treatment ",
          refDTR.substr()[2],
          ". If you are unsure, enter 0 for a conservative estimate."
        )
      )
    })
    list(
      numericInput(
        "refDTRProb.responders",
        label = HTML(
          paste0(
            "Probability of success for path <strong>",
            refDTR.substr()[2],
            ", ",
            refDTR.substr()[3],
            "</strong>"
  ...
  output$binaryCompCellProbs <- renderUI({
    ...
    validate(if (input$dyo.stage1.resprob.eq == "Yes") {
      ...
      need(
        isTruthy(input[[paste0("dyo.stage1.", compDTR.substr()[2], ".resprob")]]),
        paste0(
          "Please provide the probability of response to treatment ",
          compDTR.substr()[2],
          ". If you are unsure, enter 0 for a conservative estimate."
    ...
    list(
      ...
      bsTooltip(
        id = "compDTRProb.responders",
        title = "Input can range from 0-1 and must be in decimal form, up to two places.",
        placement = "right",
        trigger = "focus"
      ),
      bsTooltip(
        id = "compDTRProb.nonresponders",
        title = "Input can range from 0-1 and must be in decimal form, up to two places.",
        placement = "right",
        trigger = "focus"
      )
  ...
  generateProbs <- reactive({
    if (dyo.outcome() == "Binary" && input$cellOrMarginal == TRUE) {
      if (input$dyo.stage1.resprob.eq == "Yes") {
        pDTR1 <- marginalizeDTRProbs(
          input$refDTRProb.responders,
          input$dyo.stage1.allTxt.resprob,
          input$refDTRProb.nonresponders
        )
        pDTR2 <- marginalizeDTRProbs(
          input$compDTRProb.responders,
          input$dyo.stage1.allTxt.resprob,
    ...
    } else if (dyo.outcome() == "Binary" &&
               input$cellOrMarginal == FALSE) {
  ...
  output$binaryStage1Probs <- renderUI({
    if (input$dyo.stage1.ntxt > 2) {
      validate(
        need(
          isTruthy(input$dyo.stage1.Tx1),
          "Please select a first first-stage treatment to compare."
        ),
        need(
          isTruthy(input$dyo.stage1.Tx2),
          "Please select a second first-stage treatment to compare."
        )
      )
    }
    ...
    lapply(1:length(treatmentNames), function(i) {
      numericInput(
        inputId = paste0("stage1probInput", i),
  ...
  output$binaryStage2RProbs <- renderUI({
    if (input$dyo.rerand.resp.ntxt > 2) {
      validate(
        need(
          isTruthy(input$dyo.stage2R.Tx1),
          "Please select a first second-stage treatment to compare."
        ),
        need(
          isTruthy(input$dyo.stage2R.Tx2),
    ...
    lapply(1:length(treatmentNames), function(i) {
      numericInput(
        inputId = paste0("stage2RprobInput", i),
        label = paste("Success probability for Treatment",
                      treatmentNames[i]),
        value = NA
      )
  ...
  output$binaryStage2NRProbs <- renderUI({
    if (input$dyo.rerand.nresp.ntxt > 2) {
      validate(
        need(
          isTruthy(input$dyo.stage2NR.Tx1),
          "Please select a first second-stage treatment to compare."
        ),
        need(
          isTruthy(input$dyo.stage2NR.Tx2),
          "Please select a second second-stage treatment to compare."
        )
      )
    }
    ...
    lapply(1:length(treatmentNames), function(i) {
      numericInput(
        inputId = paste0("stage2NRprobInput", i),
        label = paste("Success probability for Treatment",
                      treatmentNames[i]),
        value = NA
      )
  ...
  output$contDTRInput <- renderUI({
    ...
    list(
      helpText("this is not currently functional"),
      numericInput(
        "dyo.contEffectSize",
        label = text.stdEffectLabel,
        value = NA,
        min = 0,
        max = 10,
        step = 0.01
  ...
  dyo.stage2respDiagram <-
    callModule(
      module = dyoDiagramStage2,
      id = "dyo.stage2respDiagram",
      RNR = "R",
      rerand = reactive(input$dyo.rerand.resp),
  ...
  dyo.stage2nrespDiagram <-
    callModule(
      module = dyoDiagramStage2,
      id = "dyo.stage2nrespDiagram",
      RNR = "NR",
      rerand = reactive(input$dyo.rerand.nresp),
    ...
    if (is.null(input[["dyo.primaryAim-primaryAim"]])) {
      ...
      if (input[["dyo.primaryAim-primaryAim"]] == "dtrs") {
        if (refDTR.substr()[1] != "")
          rclass <-
            paste0("class ",
                   paste0(refDTR.substr()[2:4], collapse = ","),
                   " refdtr; \n ")
        else
          rclass <- ""
        if (compDTR.substr()[1] != "")
          cclass <-
            paste0("class ",
                   paste0(compDTR.substr()[2:4], collapse = ","),
                   " compdtr; \n")
        else
          cclass <- ""
      } else {
        ...
        if (input[["dyo.primaryAim-primaryAim"]] == "stage1") {
          if (input$dyo.stage1.ntxt > 2)
            TxNames <-
              c(input$dyo.stage1.Tx1, input$dyo.stage1.Tx2)
          ...
          if (input$dyo.rerand.resp.ntxt == 2) {
            rclass <-
              paste0("class ",
                     paste0(TxNames, "r1", collapse = ","),
                     " refdtr;\n")
            # Tx2RNames2 <- paste0(TxNames, "r2", collapse = ",")
            cclass <-
              paste0("class ",
                     paste0(TxNames, "r2", collapse = ","),
                     " compdtr;\n")
          } else if (input$dyo.rerand.resp.ntxt > 2) {
          ...
          if (input$dyo.rerand.resp.ntxt == 2) {
            rclass <-
              paste0("class ",
                     paste0(TxNames, "nr1", collapse = ","),
                     " refdtr;\n")
            cclass <-
              paste0("class ",
                     paste0(TxNames, "nr2", collapse = ","),
                     " compdtr;\n")
          } else if (input$dyo.rerand.nresp.ntxt > 2) {
  ...
  output$graphstring <- renderText(
    paste(
      "graph LR;",
      "classDef default fill:#fff,stroke:#000,stroke-width:1px;",
      "classDef randomize fill:#fff,stroke:#000,stroke-width:2px;",
      "classDef compdtr fill:#fcc,stroke:#c33,stroke-width:3px,stroke-dasharray:6,4;",
      # red
      "classDef refdtr fill:#ccf,stroke:#06c,stroke-width:3px,stroke-dasharray:8,8;",
      #blue
      "R1((R));",
      "class R1 randomize;",
      dyo.stage1.diagram(),
  ...
  output$dyo.diagram <- renderDiagrammeR({
    ...
    if (input$dyo.rerand.resp == "Yes") {
      validate(need(
        length(input$dyo.rerand.resp.whichtxt) > 0,
        text.rerandMismatchResp
      ))
    } else if (input$dyo.rerand.nresp == "Yes") {
      validate(need(
        length(input$dyo.rerand.nresp.whichtxt) > 0,
        text.rerandMismatchNresp
      ))
    }
    diagram()
  })
  
  output$dyo.diagram.modal <- renderDiagrammeR({
    diagram()
  })
  ...
  observeEvent(input$zoom,
               showModal(modalDialog(
                 DiagrammeROutput(
                   "dyo.diagram.modal",
                   height = paste0(
                     60 * input$dyo.stage1.ntxt * (input$dyo.rerand.nresp.ntxt + input$dyo.rerand.resp.ntxt),
                     "px"
                   )
                 ),
  ...
  observeEvent(
    input$dyo.resultOptions.continue,
    updateCollapse(
      session,
      "dyo.setup.collapse",
  ...
  AB <- reactive({
    req(input$dyo.stage1.eqrand,
        input$dyo.stage1.resprob.eq,
        input$dyo.rerand.resp)
    ...
    if (input$dyo.stage1.eqrand == "Yes") {
      ...
      validate(# check appropriate inputs are available
        need(
          isTruthy(input[[paste0("dyo.stage1.txt",
                                 which(LETTERS == refDTR.substr()[2]),
                                 ".rprob")]]),
          paste0(
            "Please provide the probability of randomization to treatment ",
            refDTR.substr()[2],
            "."
          )
        ),
        need(
          isTruthy(input[[paste0("dyo.stage1.txt",
                                 which(LETTERS == compDTR.substr()[2]),
                                 ".rprob")]]),
          paste0(
            "Please provide the probability of randomization to treatment ",
            compDTR.substr()[2],
    ...
    if (input$dyo.stage1.resprob.eq == "Yes") {
      # Check if response probability is equal across all first-stage treatments
      validate(
        # Verify there's an input for shared response probability; if not, show message
        need(
          input$dyo.stage1.allTxt.resprob,
          "Please provide the probability of response to all first-stage treatments"
        )
      )
      r1 <-
        r0 <-
        input$dyo.stage1.allTxt.resprob  # After verifying input, store value
    } else {
      # If response probability varies by first-stage treatment,
      validate(# Check that appropriate inputs have been provided
        need(
          isTruthy(input[[paste0("dyo.stage1.",
                                 which(LETTERS == refDTR.substr()[2]),
                                 ".resprob")]]),
          paste0(
            "Please provide a response probability for first-stage treatment ",
            refDTR.substr()[2],
            "."
          )
        ),
    ...
    if (input$dyo.rerand.resp == "Yes") {
      # are any responders re-randomized in the design?
      if (LETTERS[refDTR.substr()[2]] %in% input$dyo.rerand.resp.whichtxt) {
        # are responders to the first-stage treatment recommended by the reference DTR re-randomized?
        if (input$dyo.rerand.resp.eqrand == "Yes") {
          # are responders in reference DTR re-randomized equally?
          pi2R.1 <-
            1 / input$dyo.rerand.resp.ntxt  # if yes, randomization proability is 1/(number of options)
        } else {
          # if no, pull randomization probability from the appropriate input
          validate(# check that appropriate input exists
            need(
              isTruthy(input[[paste0("dyo.rerand.resp.txt",
                                     which(LETTERS == refDTR.substr()[3]),
                                     ".rprob")]]),
              paste0(
                "Please provide a randomization probability for responders to first-stage treatment ",
                refDTR.substr()[2]
              ),
              "."
            ))
          pi2R.1 <-
            input[[paste0("dyo.rerand.resp.txt",
                          which(LETTERS == refDTR.substr()[3]),
                          ".rprob")]]
        }
      } else {
        # if responders to refDTR's first-stage treatment are not re-randomized, assignment to A2 is w/ prob 1
        pi2R.1 <- 1
      }
      if (LETTERS[compDTR.substr()[2]] %in% input$dyo.rerand.resp.whichtxt) {
        # are responders to first-stage treatment recommended by compDTR re-randomized?
        if (input$dyo.rerand.resp.eqrand == "Yes") {
          pi2R.0 <-
            1 / input$dyo.rerand.resp.ntxt  # if yes, randomization probability is 1/(number of options)
        } else {
          validate(# check that appropriate input exists
            need(
              isTruthy(input[[paste0("dyo.rerand.resp.txt",
                                     which(LETTERS == compDTR.substr()[3]),
                                     ".rprob")]]),
              paste0(
                "Please provide a randomization probability for responders to first-stage treatment ",
                compDTR.substr()[2]
          ...
          pi2R.0 <-
            input[[paste0("dyo.rerand.resp.txt",
                          which(LETTERS == compDTR.substr()[3]),
                          ".rprob")]]  # If no, pull randomization proability from appropriate input
        }
      } else {
        pi2R.0 <-
          1  # If responders to compDTR's first-stage treatment are not re-randomized, assignment to A2 is w/ prob 1
      }
    } else {
      pi2R.1 <-
        pi2R.0 <-
        1  # If no responders in the design are re-randomized, assignment to A2 is w/ prob 1 for refDTR and compDTR
    }
    
    ## Get re-randomization probabilities for non-responders
    if (input$dyo.rerand.nresp == "Yes") {
      # Are any non-responders re-randomized in the design?
      if (LETTERS[refDTR.substr()[2]] %in% input$dyo.rerand.nesp.whichtxt) {
        # If yes, are NRs to the first-stage treatment recommended by refDTR re-randomized?
        if (input$dyo.rerand.nresp.eqrand == "Yes") {
          # Are NRs re-randomized with equal probability?
          pi2NR.1 <- 1 / input$dyo.rerand.nresp.ntxt
        } else {
          # If no, pull appropriate probability
          validate(# Make sure appropriate probability is available
            need(
              isTruthy(input[[paste0("dyo.rerand.nresp.txt",
                                     which(LETTERS == refDTR.substr()[4]),
                                     ".rprob")]]),
              paste0(
                "Please provide a randomization probability for non-responders to first-stage treatment ",
                refDTR.substr()[2],
                "."
              )
            ))
          pi2NR.1 <-
            input[[paste0("dyo.rerand.nresp.txt",
                          which(LETTERS == refDTR.substr()[4]),
                          ".rprob")]]
        }
      } else {
        pi2NR.1 <-
          1  # If NRs in refDTR aren't re-randomized, assignment to A2 is w/ prob 1
      }
      if (LETTERS[compDTR.substr()[2]] %in% input$dyo.rerand.nresp.whichtxt) {
        # Are NRs to first-stage treatment recommended by compDTR re-randomized?
        if (input$dyo.rerand.nresp.eqrand == "Yes") {
          # Are they re-randomized equally?
          pi2NR.0 <-
            1 / input$dyo.rerand.nresp.ntxt  # If yes, A2 assigned w/ prob 1/(number of options)
        } else {
          validate(# check that appropriate input exists
            need(
              isTruthy(input[[paste0("dyo.rerand.nresp.txt",
          ...
          pi2NR.0 <-
            input[[paste0("dyo.rerand.nresp.txt",
                          which(LETTERS == compDTR.substr()[4]),
                          ".rprob")]]  # If not, pull from input
        }
      } else {
        pi2NR.0 <-
          1  # If NRs in compDTR aren't re-randomized, assignment to A2 w/ prob 1
      }
    } else {
      pi2NR.1 <-
        pi2NR.0 <-
        1  # If no NRs in the design are re-randomized, assignment to A2 is w/ prob 1 for refDTR and compDTR
    }
  ...
  respProb <- reactive({
    if (input$conservative) {
      ...
      if (input$dyo.stage1.resprob.eq == "Yes") {
        validate(
          need(
            isTruthy(input$dyo.stage1.allTxt.resprob),
            text.noResponse
          ) %then%
            need(
              0 <= input$dyo.stage1.allTxt.resprob &
                input$dyo.stage1.allTxt.resprob <= 1,
              text.invalidResponse
            )
        )
        return(rep(
          input$dyo.stage1.allTxt.resprob,
          input$dyo.stage1.ntxt
        ))
      } else {
        for (i in 1:input$dyo.stage1.ntxt) {
          validate(need(
            isTruthy(input[[paste0("dyo.stage1.", LETTERS[i], ".resprob")]]),
            paste0(
              "Please provide the probability of response to treatment ",
              LETTERS[i],
              "."
            ...
            need(input[[paste0("dyo.stage1.", LETTERS[i], ".resprob")]] >= 0 &
                   input[[paste0("dyo.stage1.", LETTERS[i], ".resprob")]] <= 1,
                 "hello"))
        }
  ...
  sentenceCompiler <- reactive({
    if (dyo.outcome() == "Binary" && input$cellOrMarginal == FALSE) {
      return(
        paste(
          text.sentenceAIinterest,
          input$dyoRefDTR,
          " and ",
          input$dyoCompDTR,
          text.sentenceOverallSuccess,
          input$refDTRProb,
          " and ",
          input$compDTRProb,
          ", respectively",
    ...
    if (dyo.outcome() == "Binary" &&
        input$cellOrMarginal == TRUE) {
      return(
        paste(
          text.sentenceAIinterest,
          input$dyoRefDTR,
    ...
    if (dyo.outcome() == "Continuous") {
      return(
        paste(
          text.sentenceEffectSize,
          input$firstDTRcompareA,
          " and ",
          input$secondDTRcompareA,
          ", is ",
          input$effectSizeA,
          sep = ""
        )
  ...
  output$binarySampleSize <- renderUI({
    ...
    if (isTruthy(primaryAim() == "dtrs")) {
      validate(
        ...
        need(isTruthy(compDTR.substr()[1]), text.compDTRPlaceholder)  %then%
          validate(need(
            isTruthy(input$refDTRProb),
            paste0(
              "The success probability is missing for AI ",
              refDTR.substr()[1],
              ". Please provide an input."
            )
          ),
          need(
            isTruthy(input$compDTRProb),
            paste0(
              "The success probability is missing for AI ",
              compDTR.substr()[1],
              ". Please provide an input."
    ...
    pi.stage2RA  <-
      ifelse(substr(refDTR.substr()[3], 3, 3) == "", 1,  pi2R()[as.numeric(substr(refDTR.substr()[3], 3, 3))])
    pi.stage2NRA <-
      ifelse(substr(refDTR.substr()[4], 4, 4) == "", 1, pi2NR()[as.numeric(substr(refDTR.substr()[4], 4, 4))])
    
    pi.stage1B   <- ifelse(input$dyo.stage1.eqrand == "Yes",
                           1 / input$dyo.stage1.ntxt,
                           input[[paste0("dyo.stage1.txt", which(LETTERS == compDTR.substr()[2]))]])
    pi.stage2RB  <-
      ifelse(substr(compDTR.substr()[3], 3, 3) == "", 1,  pi2R()[as.numeric(substr(compDTR.substr()[3], 3, 3))])
    pi.stage2NRB <-
      ifelse(substr(compDTR.substr()[4], 4, 4) == "", 1, pi2NR()[as.numeric(substr(compDTR.substr()[4], 4, 4))])
    
    ## Compute and round sample size
    finalSampleSize <-
      ceiling(
        sampleSize(
          alpha = dyo.resultOptions()$alpha,
          power = as.numeric(dyo.resultOptions()$inputPower),
          p1 = generateProbs()[1],
          p2 = generateProbs()[2],
    ...
    formatPower     <-
      paste(dyo.resultOptions()$inputPower * 100, "%", sep = "")
    formatAlpha     <-
      paste(dyo.resultOptions()$alpha      * 100, "%", sep = "")
    formatAltHyp    <- switch(
      dyo.resultOptions()$alternative,
      "one.sided" = "one-sided ",
      "two.sided" = "two-sided "
    )
    ...
    HTML(
      paste0(
        ...
        finalSampleSize,
        " to make this comparison with at least ",
        formatPower,
        " power. </p>"
      )
    )
  ...
  observe({
    if (input$conservative) {
      shinyBS::createAlert(
        ...
        content = paste(
          "Conservative estimates of sample size do not depend on response rates.",
          "If you have estimates of response rate(s), uncheck the box."
        )
      )
      shinyBS::createAlert(
        session,
        anchorId = "cellOrMarginalDisabled",
        alertId = "alert-conservative-cellOrMarginalDisabled",
        content = paste(
          "Cell-specific probabilities cannot be provided when",
